 ...
 * @author timheng
 */
public class JavaFXApplication11_Sep_25_1700 extends Application {
    ...
    private static final double VIEWFINDER_WIDTH = 200;
    private static final double VIEWFINDER_HEIGHT = 200;
...
//        }
    ...
    public static void makeDraggable(final Node node) {
        final JavaFXApplication11_Sep_25_1700.Delta dragDelta = new JavaFXApplication11_Sep_25_1700.Delta();
    ...
    private StackPane applyViewfinder(Node background, double width, double height, final UndirectedSparseMultigraph<Circle, Line> graph) {
        ...
        viewfinder.setOnMouseReleased(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent t) {
                double vfX = viewfinder.layoutXProperty().getValue();
                double vfY = viewfinder.layoutYProperty().getValue();
                Collection<Circle> vertices = graph.getVertices();
                for (Circle c : vertices) {
                    if ((c.getCenterX() >= vfX && c.getCenterX() <= vfX + VIEWFINDER_WIDTH) && (c.getCenterY() >= vfY && c.getCenterY() <= vfY + VIEWFINDER_HEIGHT)) {
                        PICKED.add(c);
                    }
                }
